...
import { BASE_API_URL, THREAD_LIST_ROUTE, CATEGORIES_ENDPOINT, SEARCH_THREAD_ENDPOINT } from "appConstant/apiEndpoint";
...
export function* fetchSearchThread({ type, payload }) {
  ...
  try {
    const response: AxiosResponse = yield axios.get(`${BASE_API_URL}${SEARCH_THREAD_ENDPOINT}`, {
      params: {
        search: payload.search,
      }
    })

    yield put(changeSearchThreadResult({
...
export function* fetchThreadByCategory({ type, payload }) {
  ...
  try {
    ...
    const response = yield axios.get(`${BASE_API_URL}${CATEGORIES_ENDPOINT}/${payload.categoryId}/threads`, {
      params: {
        page: payload.page || 1,
      }
    });
    ...
    yield put(changeCategoryThreads({
      threads: data.items,
    }));
...
export function* threadCreateFormSubmit ({ type, payload }) {
  ...
  try {
    ...
    if(status === 400) {
      if(data.message) {
        yield put(setFormError({ formName: 'thread-create', errors: data.message}));
      }
    }
...
export const threadSaga = [
  ...
  debounce(1200,SEARCH_THREAD, fetchSearchThread)
]
