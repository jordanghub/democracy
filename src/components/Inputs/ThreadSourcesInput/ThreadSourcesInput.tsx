...
import { Grid, TextField, Button, Typography, styled, FormHelperText } from '@material-ui/core';
...
export const ThreadSourcesInput = memo(() => {
  ...
  const handleLabelChange = (evt: any) => {
    const errors = checkErrors({ label: evt.target.value, url: inputField.value})
  ...
  const handleUrlChange = (evt: any) => {
    ...
    const errors = checkErrors({ label: labelField.value, url: evt.target.value})
    ...
    <Field name="sources">
      {
        ({ input }) => {
          ...
          const handleAddClick = () => {
            ...
            const errorField = checkErrors({ label: labelField.value, url: inputField.value});            
